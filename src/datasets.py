...
class CustDataset(Dataset):
    ...
    def prepare_all_same_names(self):
        ...
        # Get the base names of annotation files and remove extensions
        annot_files = [
        ...
        all_same_names = list(set(image_name_files).intersection(annot_files))

        incompatible_files = [
            image_name
            for image_name in image_name_files
            if image_name not in annot_files
        ]
        print(
            "All are compatible"
            if not incompatible_files
            else f"Incompatible files: {incompatible_files}"
        )
        print("----------")
    ...
    def __getitem__(self, index):
        ...
        # Read image using PIL for consistent format
    ...
    def find_image_path(self, image_name):
        return next(
            (
                os.path.join(self.img_dir, image_name + extension)
                for extension in IMAGE_TYPE
                if os.path.exists(os.path.join(self.img_dir, image_name + extension))
            ),
            None,
        )
...
if __name__ == "__main__":
    ...
    dataset = CustDataset(
        TRAIN_IMG, IMAGES_DIR, ANNOTS_DIR, RESIZE_TD[0], RESIZE_TD[1], CLASSES
    )
