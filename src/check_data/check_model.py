...
model_name = ''
# box label
true_xml_directory = './dataset/50_xml'
pred_xml_directory = './dataset/test_predictions/50_image/{model_name}/xml'
# Image check
image_directory = './dataset/50_image'
# Image compe
output_directory = './dataset/test_predictions/50_image/check_data_{model_name}'
# Specify the CSV file path
csv_output_path = './dataset/test_predictions/50_image/50_image_check_data_{model_name}.csv'
...
def process_xml_file(xml_file_path):
    ...
    for obj in root.findall('object'):
        ...
        box = [int(box_element.find(coord).text) if box_element.find(
    ...
    if os.path.exists(image_path):
        ...
        if drawn_image is not None:
            output_path = os.path.join(
                output_directory, f"{xml_filename.split('.')[0]}.png")
            pil_image = Image.fromarray(
                cv2.cvtColor(drawn_image, cv2.COLOR_BGR2RGB))
...
def process_image_and_xml(true_xml_directory, pred_xml_directory, image_directory, output_directory, csv_output_path):
    with open(csv_output_path, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        # Write CSV header
        csv_writer.writerow(
            ['Filename', 'Name', 'Box_label', 'Box_model', 'IoU'])

        # Process XML files
        for xml_filename in os.listdir(true_xml_directory):
            if xml_filename.endswith('.xml'):
                gt_xml_file_path = os.path.join(
                    true_xml_directory, xml_filename)
                pred_xml_file_path = os.path.join(
                    pred_xml_directory, xml_filename)

                if not os.path.exists(gt_xml_file_path) or not os.path.exists(pred_xml_file_path):
                    log_message(
                        f"Ground truth or predicted XML file not found for: {xml_filename}")
                    continue
                ...
                image_name, _ = os.path.splitext(
                    os.path.basename(gt_xml_file_path))
                image_path = find_image_path(image_directory, image_name)

                process_image(image_path, gt_boxes, pred_boxes,
                              output_directory, xml_filename, csv_writer)
...
with open(log_file_path, 'w') as log_file:
    ...
    sys.stdout = log_file
    process_image_and_xml(true_xml_directory, pred_xml_directory,
                          image_directory, output_directory, csv_output_path)
